...
class Conv2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
                 padding=0, norm='none', activation='relu', pad_type='zero', spectral_norm=False):
...
class MsImageDis(nn.Module):
    ...
    def _make_net(self):
        dim = self.dim
        cnn_x = []
        cnn_x += [Conv2dBlock(self.input_dim, dim, 4, 2, 1, norm=self.norm, activation=self.activ, pad_type=self.pad_type, spectral_norm=self.spectral_norm)]
        for _ in range(self.n_layer - 1):
            cnn_x += [Conv2dBlock(dim, dim * 2, 4, 2, 1, norm=self.norm, activation=self.activ, pad_type=self.pad_type, spectral_norm=self.spectral_norm)]
            dim *= 2
        if self.class_block_use:
            cnn_x += [nn.Conv2d(dim, (self.class_nums+1)*1, 4, 1, 1)]   
        else:
            cnn_x += [nn.Conv2d(dim, 2, 4, 1, 1)]   
