...
from . import networks6 as networks
...
class CycleGAN5Model(BaseModel):
    """
    ...
    def backward_D_basic(self, netD, real, real_labels, fake, fake_labels):
        """Calculate loss for the discriminator with cross-entropy and gradient penalty.
        ...
        # Flatten predictions
        preds_real = preds_real.view(preds_real.size(0), preds_real.size(1), -1).mean(2)  # [batch_size, num_classes]
        preds_fake = preds_fake.view(preds_fake.size(0), preds_fake.size(1), -1).mean(2)  # [batch_size, num_classes]
    ...
    def backward_D_A(self):
        ...
        fake_labels = torch.full((fake_B.size(0),), 6, dtype=torch.long, device=self.device)
        # Real labels
    ...
    def backward_D_B(self):
        ...
        # Fake labels (class index 6)
        fake_labels = torch.full((fake_A.size(0),), 6, dtype=torch.long, device=self.device)
        # Real labels
