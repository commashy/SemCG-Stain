...
class CycleGAN4Model(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.tvloss = TVLoss(TVLoss_weight=0.01).to(self.device)
    ...
    def backward_D_A(self):
        ...
        self.loss_D_A = self.backward_D_basic(self.netD_A, self.real_B_center, fake_B)

    def backward_D_B(self):
        """Calculate GAN loss for discriminator D_B"""
        fake_A = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B = self.backward_D_basic(self.netD_B, self.real_A_center, fake_A)
    ...
    def adaptive_threshold_light_sheet(self, image_tensor):
        """
        Apply adaptive thresholding to a grayscale light-sheet image to separate nuclei.

        Args:
            image_tensor (torch.Tensor): Grayscale light-sheet image tensor of shape (N, 1, H, W).

        Returns:
            mask (torch.Tensor): Binary mask separating nuclei from the background, shape (N, 1, H, W).
        """
        # Remove the channel dimension (dim=1)
        image_np = image_tensor.squeeze(1).cpu().detach().numpy()  # Shape: (N, H, W)
        ...
        for img in image_np:
            ...
            masks.append(thresh / 255.0)
        # Stack masks back into a tensor
        mask = torch.from_numpy(np.stack(masks)).float().to(image_tensor.device)  # Shape: (N, H, W)
