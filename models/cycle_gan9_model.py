...
class CycleGAN9Model(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        visual_names_A = ['real_A_center', 'fake_B']
        visual_names_B = ['real_B_center', 'fake_A']
        if self.isTrain:
            visual_names_A.append('rec_A_center')
            visual_names_B.append('rec_B_center')
    ...
    def forward(self):
        ...
        # G_A(A)
        self.fake_B = self.netG_A(self.real_A_center, self.embeddings_real_A)
        # G_B(B)
        self.fake_A = self.netG_B(self.real_B_center, self.embeddings_real_B)
        ...
        if self.isTrain:
            ...
            # G_B(G_A(A))
            self.rec_A_center = self.netG_B(self.fake_B, self.embeddings_fake_B)
            # G_A(G_B(B))
            self.rec_B_center = self.netG_A(self.fake_A, self.embeddings_fake_A)
    ...
    def backward_G(self):
        ...
        # Embedding loss
        self.loss_emb_A = self.criterionEmb(self.embeddings_real_A, self.embeddings_fake_B).mean()
        self.loss_emb_B = self.criterionEmb(self.embeddings_real_B, self.embeddings_fake_A).mean()
        ...
        self.loss_emb_A = torch.clamp(self.loss_emb_A, -1, 1)
        self.loss_emb_B = torch.clamp(self.loss_emb_B, -1, 1)
        ...
        self.loss_emb_A = (self.loss_emb_A + 1) / 2
        self.loss_emb_B = (self.loss_emb_B + 1) / 2
