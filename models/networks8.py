...
class SAN(nn.Module):
    ...
    def forward(self, x, stain):
        normalized = self.instance_norm(x)

        segmap = F.interpolate(stain, size=x.size()[2:], mode='nearest')
...
class MiddleBlock(nn.Module):
    def __init__(self, channels, n_heads, d_head, n_layers, d_cond):
        ...
        self.block1 = ResidualBlockWithOptionalTransformer(
            ...
            d_cond=d_cond, self_attn=True, cross=True
        )
...
class CrossUNet(nn.Module):
    def __init__(self, image_channels=3, n_channels=64,
        ...
        self.out_norm = nn.InstanceNorm2d(out_channels)
