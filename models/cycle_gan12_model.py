...
class CycleGAN12Model(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'D_B', 'G_B', 'cycle_B', 'emb_A', 'emb_B']
        ...
        if self.isTrain:
            self.model_names = ['G_A', 'G_B', 'D_A', 'D_B', 'Adapter_A']
        else:  # during test time, only load Gs
            self.model_names = ['G_A', 'G_B', 'Adapter_A']
        ...
        # Freeze netPLIP_A parameters
        for param in self.netPLIP_A.parameters():
            param.requires_grad = False
        # Freeze netPLIP_B parameters
        for param in self.netPLIP_B.parameters():
            param.requires_grad = False

        self.netAdapter_A = networks.Adapter().to(self.device)
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.netG_A.parameters(), self.netG_B.parameters(), self.netAdapter_A.parameters()), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def forward(self):
        ...
        with torch.no_grad():
            ...
            self.embeddings_real_A = self.netAdapter_A(self.embeddings_real_A)
            ...
            if self.isTrain:
                ...
                self.embeddings_real_A_center = self.netAdapter_A(self.embeddings_real_A_center)
        ...
        if self.isTrain:
            ...
            self.embeddings_fake_A = self.netAdapter_A(self.embeddings_fake_A)
            ...
            self.embeddings_fake_A_center = self.netAdapter_A(self.embeddings_fake_A_center)
    ...
    def calculate_disc_loss(self, preds, labels):
        """ preds is now a list of tensors [scale_1_pred, scale_2_pred, ...] """
        loss = 0
        # Compute the CE loss across all scales and sum them up
        for pred in preds:
    ...
    def backward_D_basic(self, netD, real, real_labels, fake, fake_labels):
        ...
        now adapted for multi-scale outputs.
        """
        ...
        # Get multi-scale predictions: Each is [batch_size, num_classes]
        preds_real = netD(real)  # preds_real is a list of outputs at different scales
        preds_fake = netD(fake)  # preds_fake is also a list

        # Compute CrossEntropyLoss across all scales
        loss_real = self.calculate_disc_loss(preds_real, real_labels)
        loss_fake = self.calculate_disc_loss(preds_fake, fake_labels)

        # Total discriminator loss (without penalty)
        loss_D = loss_real + loss_fake

        # ----- Gradient Penalty for Real Data -----
        real_class_indices = real_labels.view(-1)
        # Extract the per-sample class scores from each scale and sum them
        ...
        # Compute gradients of summed real predictions w.r.t. the real inputs
        ...
        # ----- Gradient Penalty for Fake Data -----
        fake_class_indices = fake_labels.view(-1)
        # Similarly, sum the per-sample predictions across all scales for fake data
        ...
        # Compute gradients of summed fake predictions w.r.t. the fake inputs
        ...
        # Combine the gradient penalties from real and fake
    ...
    def backward_D_A(self):
        ...
        # Fake labels (class index = last class)
