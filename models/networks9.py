...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = self.conv1(self.act1(self.norm1(self.pad1(x))))
        h = self.conv2(self.act2(self.norm2(self.pad2(h))))
...
class CrossUNet(nn.Module):
    def __init__(self, image_channels=3, n_channels=64,
        ...
        # Adapter
        self.adapter = Adapter(cross_attention_dim=512, clip_embeddings_dim=512, clip_extra_context_tokens=4)

        # Project image into feature map
        self.image_proj = nn.Sequential(
            nn.ReflectionPad2d(1),
            nn.Conv2d(image_channels, n_channels, kernel_size=3, padding=0)
        )
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            nn.ReflectionPad2d(1),
            nn.Conv2d(in_channels, image_channels, 3, padding=0),
        )
...
class NLayerDiscriminator3(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=12):
...
class Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=12, norm='none', activation='lrelu', pad_type='reflect'):
        ...
        # At this point, nf_mult represents the scaling factor for n_layers
        nf_mult_prev = nf_mult
        nf_mult = min(2 ** n_layers, 8)

        # One more convolution before the final outputs
        sequence += [
            spectral_norm(nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw)),
            nn.InstanceNorm2d(ndf * nf_mult),
            get_activation(activation)
        ...
        # The feature extractor (all layers except the final per-pixel output)
        self.feature_extractor = nn.Sequential(*sequence)

        # Per-pixel classifier (final per-pixel classification layer)
        self.per_pixel_classifier = spectral_norm(
            nn.Conv2d(ndf * nf_mult, num_classes, kernel_size=kw, stride=1, padding=padw)
        )
        # => shape [B, 1, H', W']

        # Image-level classification layers
        # Adaptive average pooling to get a single vector per image
        self.global_pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x):
        # Extract features
        features = self.feature_extractor(x)         # [B, ndf*nf_mult, H', W']

        # Per-pixel predictions
        per_pixel_out = self.per_pixel_classifier(features)  # [B, 1, H', W']

        # Image-level predictions
        pooled = self.global_pool(features)          # [B, ndf*nf_mult, 1, 1]
        pooled = pooled.view(pooled.size(0), -1)     # [B, ndf*nf_mult]

        return per_pixel_out, img_pred


class MultiScaleDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=12, num_scales=3, norm='none', activation='lrelu', pad_type='reflect'):
