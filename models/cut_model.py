...
class CUTModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.add_argument('--lambda_IDT', type=float, default=0.0, help='weight for NCE loss: NCE(G(X), X)')
    ...
    def data_dependent_initialize(self, data):
        ...
        # Weighted approach:
        if self.real_A.shape[1] == 3:  # if 3 channels (RGB)
            R = self.real_A[:, 0:1, :, :]
            G = self.real_A[:, 1:2, :, :]
            B = self.real_A[:, 2:3, :, :]
            self.real_A = 0.299*R + 0.587*G + 0.114*B
