...
class global_encoder(nn.Module):
    def __init__(self, in_channels, n_channels):
        ...
        self.emb_act = nn.SiLU()
        self.embedding = nn.Conv2d(in_channels*2, n_channels, kernel_size=3, padding=1)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x):
        h = self.init_conv(self.init_act(x))
        h = self.CBAM1(h)

        out2 = self.avg_pool(self.embedding(self.emb_act(h)))

        h = self.downsample1(self.act1(h))
        h = self.CBAM2(h)

        h = self.downsample2(self.act2(h))
        h = self.CBAM3(h)

        h = self.downsample3(self.act3(h))
        out1 = self.CBAM4(h)
...
class CrossUNet(nn.Module):
    def __init__(self, image_channels=3, n_channels=64,
                 ...
                 cross=(False, False, False, True, True, True), #changed
    ...
    def forward(self, x_large, full=False):
        ...
        # Generate image embedding from the larger input with positional embeddings
        if full:
            x_global, cond2 = self.global_encoder(x_large)
        else:
            fake_x_large = F.interpolate(x_large, scale_factor=3, mode='bilinear')
            x_global, cond2 = self.global_encoder(fake_x_large)
