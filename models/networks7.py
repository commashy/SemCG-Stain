...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal',
    ...
    if netG == 'resnet_9blocks':
        ...
        net = ResnetGenerator2(input_nc, output_nc, ngf, use_dropout=use_dropout, n_blocks=9)
    else:
...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, no_antialias=False, gpu_ids=[], opt=None, num_classes=None):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        ...
        net = NLayerDiscriminator3(input_nc, ndf, n_layers=3, norm_layer=norm_layer, no_antialias=no_antialias, num_classes=num_classes)
    else:
...
class Downsample(nn.Module):
    def __init__(self, n_channels):
        super().__init__()
        self.conv = nn.Conv2d(n_channels, n_channels, 3, padding=1, stride=2)
...
class GCN(nn.Module):
    def __init__(self, n_channels):
        ...
        self.mlp_shared = nn.Sequential(
            nn.Conv2d(64, nhidden, kernel_size=ks, padding=pw),
            nn.SiLU(),
        )
    ...
    def forward(self, x, emb):
        ...
        # Expand emb to match spatial dimensions
        emb = emb.expand(-1, -1, x.size(2), x.size(3))  # Shape: (N, emb_channels, H, W)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        # self.norm1 = GCN(in_channels)
        self.norm1 = nn.InstanceNorm2d(in_channels)
        self.act1 = nn.SiLU()
        self.conv1 = nn.Conv2d(in_channels, out_channels, 3, padding=1)

        # self.norm2 = GCN(out_channels)
...
class ResidualBlockWithOptionalTransformer(nn.Module):
    def __init__(self, in_channels, out_channels, use_transformer=False,
        ...
        #     )
        # else:
        #     self.transformer_block2 = None

    def forward(self, x, cond1=None, cond2=None):
        h = self.res_block1(x, cond2)
        if self.use_transformer:
            h = self.transformer_block1(h, cond1) + h
    
        # h = self.res_block2(h, cond2)
        # if self.use_transformer:
        #     h = self.transformer_block2(h, cond1) + h
...
class MiddleBlock(nn.Module):
    def __init__(self, channels, n_heads, d_head, n_layers, d_cond):
        ...
        self.block1 = ResidualBlockWithOptionalTransformer(
            ...
            d_cond=d_cond, self_attn=True, cross=False
        )
    ...
    def forward(self, x, cond1=None, cond2=None):
        x = self.block1(x, cond1=cond1, cond2=cond2)
        x = self.block2(x, cond1=cond1, cond2=cond2)

        return x

class ResBlockwon(nn.Module):
...
class global_encoder(nn.Module):
    def __init__(self, in_channels, n_channels):
        ...
        self.init_conv = nn.Conv2d(in_channels, in_channels*2, kernel_size=7, padding=3, stride=3) # 6, 256x256
        self.res1 = ResBlockwon(in_channels*2)

        self.act1 = nn.SiLU()
        self.downsample1 = nn.Conv2d(in_channels*2, in_channels*4, kernel_size=7, padding=3, stride=2) # 12, 128x128
        self.res2 = ResBlockwon(in_channels*4)

        self.act2 = nn.SiLU()
        self.downsample2 = nn.Conv2d(in_channels*4, in_channels*8, kernel_size=7, padding=3, stride=2) # 24, 64x64
        self.res3 = ResBlockwon(in_channels*8)

        self.act3 = nn.SiLU()
        self.downsample3 = nn.Conv2d(in_channels*8, n_channels, kernel_size=7, padding=3, stride=2) # 64, 32x32
        self.res4 = ResBlockwon(n_channels)

        self.emb_act = nn.SiLU()
        self.emb_conv = nn.Conv2d(in_channels*2, n_channels, kernel_size=3, padding=1, stride=2)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x):
        h = self.res1(self.init_conv(self.init_act(x)))

        out2 = self.avg_pool(self.emb_conv(self.emb_act(h)))

        h = self.res2(self.downsample1(self.act1(h)))

        h = self.res3(self.downsample2(self.act2(h)))

        out1 = self.res4(self.downsample3(self.act3(h)))
...
class CrossUNet(nn.Module):
    def __init__(self, image_channels=3, n_channels=64,
                 ...
                 is_attn=(False, False, False, True, True, True), #change
                 self_attn=(False, False, False, True, True, True), #changed
                 cross=(False, False, False, False, False, False), #changed
    ...
    def forward(self, x_large, full=False):
        ...
        if full:
            cond1, cond2 = self.global_encoder(x_large)
        else:
            fake_x_large = F.interpolate(x_large, scale_factor=3, mode='bilinear')
            cond1, cond2 = self.global_encoder(fake_x_large)

        # Flatten x_global for conditioning
        b, c, h, w = cond1.shape
        cond1 = cond1.view(b, c, h * w).permute(0, 2, 1)  # [batch_size, n_cond, d_cond]
        ...
        for m in self.down:
            if isinstance(m, Downsample):
                ...
                if m.use_transformer:
                    x = m(x, cond1=cond1, cond2=cond2)
                else:
                    x = m(x, cond2=cond2)
                h.append(x)

        # Middle block
        x = self.middle(x, cond1=cond1, cond2=cond2)
...
class ResnetBlockWithACN(nn.Module):
    """Resnet block with ACN"""
    def __init__(self, dim, group):
        super(ResnetBlockWithACN, self).__init__()
        self.bottleneck_dim = dim*2

        # Define convolutional layers
        self.conv1 = nn.Conv2d(dim, self.bottleneck_dim, kernel_size=1)
        self.conv2 = nn.Conv2d(self.bottleneck_dim, dim, kernel_size=1, bias=False)

        self.group_conv = nn.Conv2d(self.bottleneck_dim, self.bottleneck_dim, kernel_size=3, padding=1, groups=group)

        # Define ACN layers
        self.acn1 = ACN(dim)
        self.acn2 = ACN(self.bottleneck_dim)
        self.acn3 = ACN(self.bottleneck_dim)
    ...
    def forward(self, x, emb):
        ...
        out = self.acn1(out, emb)
        out = self.act1(out)
        out = self.conv1(out)

        # Group convolution
        out = self.acn2(out, emb)
        out = self.act2(out)
        out = self.group_conv(out)

        out = self.acn3(out, emb)
        out = self.act3(out)
        out = self.conv2(out)

        return x + out  # Residual connection

# Resnet Block without bottleneck
class ResnetBlockWithoutBottleneck(nn.Module):
    def __init__(self, dim, use_dropout):
        super(ResnetBlockWithoutBottleneck, self).__init__()
...
# Updated ResnetGenerator
class ResnetGenerator2(nn.Module):
    """Resnet-based generator with SPADE normalization in all layers"""

    def __init__(self, input_nc, output_nc, ngf=64, use_dropout=False, n_blocks=6, padding_type='reflect'):
        """Construct a Resnet-based generator with SPADE normalization"""
        ...
        self.initial = nn.Sequential(
            nn.Conv2d(input_nc, ngf, kernel_size=7, padding=3, bias=use_bias, padding_mode='reflect'),
        )
        ...
        for i in range(n_downsampling):
            ...
            self.down_layers.append(nn.ModuleDict({
                'conv': nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1),
                'res': ResnetBlockWithACN(out_channels, group),
            }))
        ...
        for i in range(n_blocks):
            self.res_blocks.append(
                ResnetBlockWithACN(ngf * mult, group)
            )
    ...
    def forward(self, input, full=False):
        if full:
            ...
            x = input[:, :, start:end, start:end]
        else:
            x = input

        emb = self.emb_layer(input)  # Use the full input for embedding
        x = self.initial(x)
...
class NLayerDiscriminator3(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=13):
        ...
        # Global pooling and fully connected layer with spectral normalization
        self.global_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = spectral_norm(nn.Linear(ndf * nf_mult, num_classes))
