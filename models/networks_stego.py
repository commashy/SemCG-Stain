...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[], num_classes=None):
...
class Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=12, norm='none', activation='lrelu', pad_type='reflect'):
        ...
        self.per_pixel_classifier = spectral_norm(nn.Conv2d(ndf * nf_mult, num_classes, kernel_size=kw, stride=1, padding=padw))
    ...
    def forward(self, x):
        ...
        # per_pixel_out = self.per_pixel_classifier(features)  # [B, num_classes, H', W']

        # Image-level predictions
        pooled = self.global_pool(features)          # [B, ndf*nf_mult, 1, 1]
        pooled = pooled.view(pooled.size(0), -1)     # [B, ndf*nf_mult]
        img_pred = self.fc(pooled)                   # [B, num_classes]
...
class MultiScaleDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, num_classes=12, num_scales=3, norm='none', activation='lrelu', pad_type='reflect'):
        ...
        for _ in range(self.num_scales):
            discriminator = Discriminator(input_nc, ndf, n_layers=3, num_classes=num_classes)
